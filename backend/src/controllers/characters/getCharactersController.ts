...
export default asyncHandler(async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const cachedData = await redisClient.get(`characters:page:${page}:status:${status}:gender:${gender}`);

    if (cachedData) {
      data = JSON.parse(cachedData) as dataApiCharactersListResponse;
    } else {
      const response = await fetch(`${process.env.DATA_API_URL}/?page=${page}${status === 'all' ? '' : `&status=${status}`}${gender === 'all' ? '' : `&gender=${gender}`}`);
      ...
      await redisClient.set(`characters:page:${page}:status:${status}:gender:${gender}`, JSON.stringify(data), 'EX', process.env.REDIS_EXPIRATION || 60 * 60 * 24);
    }

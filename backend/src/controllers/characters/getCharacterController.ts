...
export default asyncHandler(async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (cachedData) {
      ...
      const response = await fetch(`${process.env.DATA_API_URL}/${characterId}`);
      ...
      await redisClient.set(`character:${characterId}`, JSON.stringify(data), 'EX', process.env.REDIS_EXPIRATION || 60 * 60 * 24);
    }
